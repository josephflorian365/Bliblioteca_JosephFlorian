...
public class MainController implements Initializable, BookReturnCallback {
    ...
    private static final String BOOK_NOT_AVAILABLE = "No disponible";
    private static final String NO_SUCH_BOOK_AVAILABLE = "No existe tal libro disponible";
    private static final String NO_SUCH_MEMBER_AVAILABLE = "Miembro no disponible";
    private static final String BOOK_AVAILABLE = "Estado";
    ...
    private void loadIssueOperation(ActionEvent event) {
        if (checkForIssueValidity()) {
            ...
            AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(btn), "entrada inválida", null);
            return;
        }
        if (bookStatus.getText().equals(BOOK_NOT_AVAILABLE)) {
            JFXButton btn = new JFXButton("Okay!");
            JFXButton viewDetails = new JFXButton("Ver detalles");
        ...
        yesButton.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent event1) -> {
            ...
            if (databaseHandler.execAction(str) && databaseHandler.execAction(str2)) {
                ...
                AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(button), "Emisión del libro completa", null);
                refreshGraphs();
            } else {
                JFXButton button = new JFXButton("De acuerdo. Comprobaré");
                AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(button), "La operación de problema falló", null);
            }
            clearIssueEntries();
        });
        JFXButton noButton = new JFXButton("NO");
        noButton.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent event1) -> {
            JFXButton button = new JFXButton("Esta bien");
            AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(button), "Problema cancelado", null);
            clearIssueEntries();
        });
        AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(yesButton, noButton), "Confirm Issue", "¿Seguro que quieres emitir el libro? " + bookName.getText() + " a " + memberName.getText() + " ?");
    }
    ...
    private void loadBookInfo2(ActionEvent event) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                JFXButton button = new JFXButton("De acuerdo. Comprobaré");
                AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(button), "No existe tal libro en la base de datos", null);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void loadSubmissionOp(ActionEvent event) {
        if (!isReadyForSubmission) {
            JFXButton btn = new JFXButton("Okay!");
            AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(btn), "Seleccione un libro para enviar", "No puedo simplemente enviar un libro nulo :-)");
            return;
        }

        JFXButton yesButton = new JFXButton("SI, Por favor");
        yesButton.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent ev) -> {
            ...
            if (databaseHandler.execAction(ac1) && databaseHandler.execAction(ac2)) {
                JFXButton btn = new JFXButton("hehco!");
                AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(btn), "El libro ha sido enviado", null);
                disableEnableControls(false);
                submissionDataContainer.setOpacity(0);
            } else {
                JFXButton btn = new JFXButton("De acuerdo. Comprobaré");
                AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(btn), "La presentación ha sido reprobada", null);
            }
        });
        JFXButton noButton = new JFXButton("No, Cancelar");
        noButton.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent ev) -> {
            JFXButton btn = new JFXButton("Okay!");
            AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(btn), "Envío de la operación cancelada", null);
        });

        AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(yesButton, noButton), "Confirmar la operación de envío", "¿Seguro que quieres devolver el libro? ?");
    }
    ...
    private void loadRenewOp(ActionEvent event) {
        if (!isReadyForSubmission) {
            JFXButton btn = new JFXButton("Okay!");
            AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(btn), "Seleccione un libro para renovar", null);
            return;
        }
        JFXButton yesButton = new JFXButton("YES, Por favor");
        yesButton.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent event1) -> {
            String ac = "UPDATE ISSUE SET issueTime = CURRENT_TIMESTAMP, renew_count = renew_count+1 WHERE BOOKID = '" + bookID.getText() + "'";
            System.out.println(ac);
            if (databaseHandler.execAction(ac)) {
                JFXButton btn = new JFXButton("Bien!");
                AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(btn), "El libro ha sido renovado", null);
                disableEnableControls(false);
                submissionDataContainer.setOpacity(0);
            } else {
                JFXButton btn = new JFXButton("Okay!");
                AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(btn), "Renovar se ha fallado", null);
            }
        });
        JFXButton noButton = new JFXButton("No, No lo hagas!");
        noButton.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent event1) -> {
            JFXButton btn = new JFXButton("Okay!");
            AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(btn), "Renovar operación cancelada", null);
        });
        AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(yesButton, noButton), "Confirmar renovar operación", "¿Seguro que quieres renovar el libro??");
    }
    ...
    private void handleMenuAddBook(ActionEvent event) {
        LibraryAssistantUtil.loadWindow(getClass().getResource("/library/assistant/ui/addbook/add_book.fxml"), "Agregar nuevo libro", null);
    }

    @FXML
    private void handleMenuAddMember(ActionEvent event) {
        LibraryAssistantUtil.loadWindow(getClass().getResource("/library/assistant/ui/addmember/member_add.fxml"), "Agregar nuevo miembro", null);
    }

    @FXML
    private void handleMenuViewBook(ActionEvent event) {
        LibraryAssistantUtil.loadWindow(getClass().getResource("/library/assistant/ui/listbook/book_list.fxml"), "Lista de libros", null);
    }

    @FXML
    private void handleAboutMenu(ActionEvent event) {
        LibraryAssistantUtil.loadWindow(getClass().getResource("/library/assistant/ui/about/about.fxml"), "Sobre mi", null);
    }

    @FXML
    private void handleMenuSettings(ActionEvent event) {
        LibraryAssistantUtil.loadWindow(getClass().getResource("/library/assistant/settings/settings.fxml"), "Configuraciones", null);
    }

    @FXML
    private void handleMenuViewMemberList(ActionEvent event) {
        LibraryAssistantUtil.loadWindow(getClass().getResource("/library/assistant/ui/listmember/member_list.fxml"), "Lista de miembros", null);
    }

    @FXML
    private void handleIssuedList(ActionEvent event) {
        Object controller = LibraryAssistantUtil.loadWindow(getClass().getResource("/library/assistant/ui/issuedlist/issued_list.fxml"), "Lista de libros emitidos", null);
        if (controller != null) {

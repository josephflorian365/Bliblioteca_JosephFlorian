    ...
    public static void initPDFExprot(StackPane rootPane, Node contentPane, Stage stage, List<List> data) {
        ...
        openBtn.setOnAction((ActionEvent event1) -> {
            try {
                ...
                AlertMaker.showErrorMessage("No se pudo cargar el archivo", "No se puede cargar el archivo");
            }
        });
        if (flag) {
            AlertMaker.showMaterialDialog(rootPane, contentPane, Arrays.asList(okayBtn, openBtn), "Terminado", "Los datos de los miembros han sido exportados.");
        }
    }

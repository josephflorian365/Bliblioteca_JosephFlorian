...
public class MemberListController implements Initializable {
    ...
    private void handleMemberDelete(ActionEvent event) {
        ...
        if (selectedForDeletion == null) {
            AlertMaker.showErrorMessage("Ningún miembro seleccionado", "Seleccione un miembro para su eliminación.");
            return;
        }
        if (DatabaseHandler.getInstance().isMemberHasAnyBooks(selectedForDeletion)) {
            AlertMaker.showErrorMessage("No se puede eliminar", "Este miembro tiene algunos libros.");
            return;
        }
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setTitle("Borrando el libro");
        alert.setContentText("¿Seguro que quieres eliminar " + selectedForDeletion.getName() + " ?");
        Optional<ButtonType> answer = alert.showAndWait();
        if (answer.get() == ButtonType.OK) {
            Boolean result = DatabaseHandler.getInstance().deleteMember(selectedForDeletion);
            if (result) {
                AlertMaker.showSimpleAlert("Libro borrado", selectedForDeletion.getName() + " fue eliminado con éxito.");
                list.remove(selectedForDeletion);
            } else {
                AlertMaker.showSimpleAlert("Ha fallado", selectedForDeletion.getName() + " no se pudo borrar");
            }
        } else {
    ...
    private void handleMemberEdit(ActionEvent event) {
        ...
        if (selectedForEdit == null) {
            AlertMaker.showErrorMessage("Ningún miembro seleccionado", "Seleccione un miembro para editar.");
            return;
        }
        try {
    ...
    private void exportAsPDF(ActionEvent event) {
        ...
        String[] headers = {"   Nombre    ", "ID", "Celular", "    Email   "};
